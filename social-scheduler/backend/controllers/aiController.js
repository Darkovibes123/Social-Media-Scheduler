// Uses Google Gemini's free-tier API (no paid billing required to start).
...
exports.generateCaption = async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(500).json({ message: "Server is missing GEMINI_API_KEY in .env" });
    }

    const prompt = `You are a social media assistant for a student influencer/campus creator.
